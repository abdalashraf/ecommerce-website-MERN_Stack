...
const OrderTable = () => {
  ...
  const  orderDelete = (orderDelete)=>{
    axios.delete(`/orderdelete?anc=${orderDelete}`).then(function(res){
      if(res){
        toast.success("Order Deleted SuccessFully")
      }
      setOrder(order.filter(pro=>pro._id !=orderDelete))
  console.log(res);
}).catch ( function(error){
  console.log(error);
})
 ...
 <tr>
   ...
   <td><AiFillDelete className='deleteButton' onClick={() => orderDelete(order._id)} /></td>
 </tr>
